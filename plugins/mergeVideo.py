...
async def mergeNow(c: Client, cb: CallbackQuery, new_file_name: str):
    """Enhanced merge function with better process management"""
    ...
    try:
        ...
        for i in await c.get_messages(
            ...
            await cb.message.edit(f"📥 Downloading ({n}/{all}): `{media.file_name}`")
            ...
            try:
                ...
                await cb.message.edit(f"✅ Downloaded: `{media.file_name}`")
        ...
        await cb.message.edit(
            f"🔄 Renamed to: `{new_file_name.rsplit('/',1)[-1]}`"
        )
        await asyncio.sleep(3)
